...
from Mithun_Protocol_SR import rdt_Sender, rdt_Receiver
...
channel_for_data  = UnreliableChannel(env=env,name="DATA_CHANNEL",Pc=0.4,Pl=0.4,propagation_delay=2, transmission_rate=1000)
channel_for_ack	  = UnreliableChannel(env=env,name="ACK_CHANNEL", Pc=0.4,Pl=0.4,propagation_delay=2, transmission_rate=1000)


# Set some parameters for the Go-Back-N Protocol
rdt_sender.N=6# Window size for the sender
rdt_receiver.N=6 # Window size for the receiver (Note: This is ignored in the GBN protocol, but required in the SR protocol)
rdt_sender.K=8 # Packet sequence numbers range from 0 to K-1
rdt_receiver.K=8 # Packet sequence numbers range from 0 to K-1
